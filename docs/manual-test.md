...
> 出处 `docs/ch09/checklist.md` 的 T14 与 `docs/ch09/ui-align-plan.md`。对齐 PR #2 的色板与输入框观感，
> 并复验真机暴露的「页脚残留」缺陷。改了源码必须重新打包（`JAVA_HOME=D:\java\jdk21 mvn -DskipTests package`）。

- [ ] ⚑ 启动后等输入时：提示符为 `>*`；**上方**一行模式行（亮黄模式名 + 暗灰提示）与一行分隔线；
  **下方**一行分隔线 + 一行页脚（模型青色 · 10 格 ctx 进度条 · 百分比 · 工作目录）。
- [ ] ⚑ 输入 `/permission acceptEdits` → 下一次等输入时模式行显示 `[acceptEdits]`。
- [ ] ⚑ **残留缺陷复验**：对话一轮（屏上出现 `usage:` 脚注）后敲 `/help` → `/help` 的输出
  **不与上一轮内容糊在一起**；敲之前那一瞬间位于提示符下方的页脚两行已被擦掉，屏上无孤儿行、无残影。
- [ ] ⚑ 连按几次空回车：模式行与分隔线**不重复堆叠**（空行只在原地等下一次输入）。
- [ ] ⚑ 六种情形各走一遍——**改过窗口尺寸 / 输出超过一屏 / `/clear` / `/resume` / 空行回车 /
  流式中 Ctrl+C**：提示符位置正确、帧不错位、无残影。
- [ ] ⚑ **多行/长输入与页脚的关系**（设计上的已知风险，务必看一眼）：Shift+Enter 输入两行内容、
  或输入长到自动折行的内容，再回车提交。页脚画在提示符**下方**，而 JLine 画提示符时会向下占用行，
  两者可能重叠（PR 那套设计本身就有这个性质）。判据：提交后屏上**不留**被压掉一半的页脚残行，
  下一轮帧的位置仍然正确。若重叠且留有残影，记下复现输入长度，这是需要回头改设计的真实缺陷。
- [ ] ⚑ 宽度收窄到放不下时，模式行与页脚**截断而不折行**（用长中文路径的目录名试）。
  **另注**：进度条的 `▓░`、分隔线的 `─`、路径省略号 `…` 都是东亚「模糊宽度」字符——单测按 1 列算，
  若本机终端把它们渲染成双宽，页脚会实际超出宽度而折成两行、帧随即错位。这一条主要就是在真机上
...
- [ ] ⚑ 重跑 `JAVA_HOME=D:\java\jdk21 mvn test` 全绿（记录总用例数，与 CMD7 的 1008 对比）。→ **1056 用例，0 失败 / 0 错误 / 1 跳过**（2026-09-14）
