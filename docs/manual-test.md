...
>
> **布局语义（验收基准）**：输入框「**上方紧贴对话内容、下方紧贴屏幕底部**」。提示符上方的
> 模式行 + 分隔线进回滚、随输出滚动；提示符下方的分隔线 + 页脚钉在屏幕底部。**内容少时
> （如刚启动）两者之间的空档是预期现象，不是缺陷**——输出填满滚动区后提示符就落到贴近页脚的位置。
...
- [ ] ⚑ 启动后等输入时：提示符 `>*` **上方**一行模式行（亮黄模式名 + 暗灰 `/permission <模式>`）与
