...
- [ ] ⚑ 输入 `/` 按 Tab → 列出全部可见命令（含 `/quit`，每条带描述）。
- [ ] ⚑ 输入 `/com` 按 Tab → 直接补全为 `/compact`（不提交）。
- [ ] ⚑ 多候选时（如 `/p` 按 Tab）→ 呈现方式符合预期、可继续选择、接受后输入行干净。
- [ ] ⚑ 补全后继续输入参数 → 输入行正常；按 Esc / Ctrl+C → 输入行与回滚区正确清理。
- [ ] ⚑ 滚动回滚区 → 无补全列表残影。
- [ ] ⚑ `docs/ch09/spec.md` 的「关键设计决策」表中 Tab 补全行已填入结论与所选方案。
...
- [ ] ⚑ 假 provider 端到端：全部可见命令逐个执行，**按 `docs/ch09/checklist.md` 顶部「命令类型与通道」表**核对每种类型的请求条数与去向（本地命令的对话请求恒为 0，但 `/compact`、`/memory run` 的专用请求各计一次；`/plan <任务>` 与 `/do`（有计划）各发一次对话请求；提示词命令只发对话请求）。
- [ ] ⚑ 全程 `JAVA_HOME=D:\java\jdk21 mvn test` 全绿（记录总用例数）。
